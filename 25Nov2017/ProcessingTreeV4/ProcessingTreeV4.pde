...
void mouseClicked() {
  ...
  startPower = 60 + spread/360 *130;//higher branch chances require more power to not get tiny
  background(200,210,230);
  if (numTrees > 1) {
    for(int i = numTrees;i>0;i--) {
      spread = 20 + random(240);
      drawNode(random(width),height, startPower + random(startPower/2),random(spread)-spread/2); 
      fill(200,210,230,70);
      noStroke();
